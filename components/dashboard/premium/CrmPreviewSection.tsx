...
function getAiTier(score: number): { label: string; className: string } {
  if (score >= 90) return { label: "Hot", className: "text-rose-400 bg-rose-500/15 border-rose-400/30" };
  if (score >= 80) return { label: "High", className: "text-orange-400 bg-orange-500/15 border-orange-400/30" };
  if (score >= 70) return { label: "Medium", className: "text-amber-400 bg-amber-500/15 border-amber-400/30" };
  return { label: "Low", className: "text-gray-400 bg-gray-500/15 border-gray-400/30" };
}
    ...
    >
      <div className="flex items-center justify-between gap-3 mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Pipeline & revenue at a glance</p>
        </div>
        <Link
          href="/dashboard/crm"
      ...
      <div className="grid sm:grid-cols-2 gap-5">
        <div>
          <p className="text-xs text-gray-500 uppercase tracking-wider mb-3">Hot Leads</p>
        ...
        <div>
          <p className="text-xs text-gray-500 uppercase tracking-wider mb-3">Top Customers</p>
          <ul className="space-y-2">
