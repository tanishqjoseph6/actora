...
POST /api/v1/roxx/workflows`} />
          ...
          <section id="limits" className="scroll-mt-28 py-14">
            ...
            <div className="grid gap-3 sm:grid-cols-3">{[["Free", "5 GB storage", "250 calls / month", "20 requests / minute"], ["Pro", "50 GB storage", "1,500 calls / month", "300 requests / minute"], ["Team", "250 GB storage", "5,000 calls / month", "500 requests / minute"]].map(([plan, storage, calls, rate]) => <div key={plan} className="rounded-xl border border-white/[0.08] bg-[#111111] p-4"><p className="font-medium text-white">{plan}</p><p className="mt-3 text-sm text-[#A1A1AA]">{storage}</p><p className="mt-1 text-sm text-[#A1A1AA]">{calls}</p><p className="mt-1 text-sm text-[#60A5FA]">{rate}</p></div>)}</div>
            <p className="mt-5 text-sm text-[#71717A]">Signed-in workspace usage, API key status, and usage history are available from <Link href="/dashboard/settings#api-usage" className="text-[#93C5FD] hover:text-white">Workspace Usage</Link>.</p>
