...
export async function POST(_request: NextRequest, context: RouteContext) {
  ...
  try {
    const workflow = await automationRepository.pauseWorkflow(userId, id, userId);
    ...
    console.error("[automations/pause] failed:", error);
    return apiError(error instanceof Error ? error.message : "Failed to pause workflow.");
  }
}
